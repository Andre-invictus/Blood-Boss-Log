...
function compareView(selected=[]){const chosen=new Set(selected);return title()+`<p class="sub">Selecione contas usando Ctrl ou Command.</p><select id="cmp" multiple style="width:100%;min-height:180px;background:#111;color:#fff;border:1px solid #333;padding:10px">${S.scores.map(x=>`<option ${chosen.has(x.player)?'selected':''}>${esc(x.player)}</option>`).join('')}</select><button id="cmpBtn" class="btn blue" style="margin-top:10px">Comparar</button><div id="cmpOut"></div>`}
function compareResults(names){return '<div class="combat" style="margin-top:15px">'+S.scores.filter(x=>names.includes(x.player)).map(x=>`<div class="row"><span class="player">${esc(x.player)}</span><span class="stats"><b class="blue">${esc(x.blue)}</b><b class="red">${esc(x.red)}</b></span></div>`).join('')+'</div>'}
...
async function editActivity(id){
  ...
  const {error}=await sb.from('public_activity').update({action:action.trim(),entity_name:name.trim()}).eq('id',id);if(error)return alert(error.message);alert(ep('Atividade atualizada.','Activity updated.','Actividad actualizada.','动态已更新。','Đã cập nhật hoạt động.'));await extLoad();render();
}
async function deleteActivity(id){
  ...
  const {error}=await sb.from('public_activity').delete().eq('id',id);if(error)return alert(error.message);alert(ep('Atividade apagada.','Activity deleted.','Actividad eliminada.','动态已删除。','Đã xóa hoạt động.'));await extLoad();render();
}
...
const oldRender=render;render=()=>{if(S.tab==='myProfile'){$('#main').innerHTML=profileView();return}if(S.tab==='rankings'){$('#main').innerHTML=rankingsView();return}if(S.tab==='compare'){
  const selectedNow=$('#cmp')?[...$('#cmp').selectedOptions].map(x=>x.value):(S.compareSelected||[]);
  const hadResult=!!S.compareHasResult;
  const scrollPosition=window.scrollY;
  S.compareSelected=selectedNow;
  $('#main').innerHTML=compareView(selectedNow);
  if(hadResult&&selectedNow.length)$('#cmpOut').innerHTML=compareResults(selectedNow);
  $('#cmp').onchange=()=>{S.compareSelected=[...$('#cmp').selectedOptions].map(x=>x.value)};
  $('#cmpBtn').onclick=()=>{S.compareSelected=[...$('#cmp').selectedOptions].map(x=>x.value);S.compareHasResult=true;$('#cmpOut').innerHTML=compareResults(S.compareSelected);translateEnhancements($('#main'))};
  requestAnimationFrame(()=>{window.scrollTo(0,scrollPosition);translateEnhancements($('#main'))});
  return
...
function addAdminTools(){
  ...
  const refreshLists=()=>{const accountList=$('#paList'),acList=$('#acList'),goList=$('#goList');if(accountList)accountList.innerHTML=X.accounts.length?X.accounts.map(a=>`<div class="adm-list-item"><span><b>${esc(a.nickname)}</b><small class="sub"> ${esc(a.user_id)}</small></span><span class="tag">${a.active?'ATIVO':'INATIVO'}</span></div>`).join(''):'<div class="adm-empty">Nenhum vínculo encontrado.</div>';if(acList)acList.innerHTML=X.achievements.length?X.achievements.map(a=>`<div class="adm-list-item"><span><b>${esc(a.title)}</b><small class="sub"> ${esc(a.period)} · ${esc(a.metric)}</small></span><b>${esc(a.threshold_k)}k</b></div>`).join(''):'<div class="adm-empty">Nenhuma conquista ativa.</div>';if(goList)goList.innerHTML=X.goals.length?X.goals.map(g=>`<div class="adm-list-item"><span><b>${esc(g.title)}</b><small class="sub"> ${esc(g.period)}</small></span><b>${esc(g.target_k)}k</b></div>`).join(''):'<div class="adm-empty">Nenhuma meta ativa.</div>'};
  refreshLists();
...
const previousAdminOpen=$('#admOpen')&&$('#admOpen').onclick;
if($('#admOpen'))$('#admOpen').onclick=()=>{ensureAdminTools();if(previousAdminOpen)previousAdminOpen();else $('#admin').showModal()};
