...
(()=>{
 const A={id:null,row:null,channel:null,lastPage:'',started:false,geo:null};
 ...
 const browser=()=>{const u=navigator.userAgent;return /Edg\//.test(u)?'Edge':/Chrome\//.test(u)?'Chrome':/Firefox\//.test(u)?'Firefox':/Safari\//.test(u)?'Safari':'Outro'};
 ...
 async function start(){if(A.started||!S.profile)return;A.started=true;A.id=crypto.randomUUID?crypto.randomUUID():Date.now()+'-'+Math.random();A.geo=await context();const now=new Date().toISOString(),payload={session_id:A.id,user_id:S.profile.id,started_at:now,last_seen_at:now,current_page:page(),language:S.lang,device_type:device(),browser_name:browser(),os_name:os(),timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'',country_code:A.geo.country||null,region_code:A.geo.region||null,city_name:A.geo.city||null,latitude:A.geo.latitude||null,longitude:A.geo.longitude||null};const {data,error}=await sb.from('access_sessions').insert(payload).select('id').single();if(error){console.warn('Access session:',error);return}A.row=data.id;presence();view(true);setInterval(heartbeat,60000);setInterval(()=>view(false),1500)}
 ...
 function state(){return{user_id:S.profile.id,display_name:S.profile.display_name||'',role:S.profile.role,page:page(),session_id:A.id,online_at:new Date().toISOString(),device_type:device(),country_code:A.geo?.country||null,region_code:A.geo?.region||null,city_name:A.geo?.city||null,latitude:A.geo?.latitude||null,longitude:A.geo?.longitude||null}}
 function presence(){A.channel=sb.channel('blood-site-presence',{config:{presence:{key:A.id}}});A.channel.subscribe(async s=>{if(s==='SUBSCRIBED')await A.channel.track(state())})}
