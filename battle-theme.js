...
body.battle-theme #battleBanner{display:flex;align-items:center;justify-content:center;text-align:center}#battleBanner b{font:italic 1.35rem Teko;letter-spacing:.06em;width:100%}
`;
  const SERVER_TZ='America/Sao_Paulo',CACHE_KEY='blood_battle_theme_mode_v23';
  function serverDay(){const parts=new Intl.DateTimeFormat('en-CA',{timeZone:SERVER_TZ,day:'2-digit'}).formatToParts(new Date());return Number(parts.find(x=>x.type==='day')?.value||0)}
  function calendar(){return DAYS.includes(serverDay())}
  ...
  window.BloodBattleTheme={reload:async()=>{await load()},apply};
  async function load(){try{const {data,error}=await sb.from('battle_theme_settings').select('mode,updated_at').eq('id',true).single();if(error)throw error;if(data&&['auto','on','off'].includes(data.mode)){state.mode=data.mode;localStorage.setItem(CACHE_KEY,state.mode)}}catch(e){const cached=localStorage.getItem(CACHE_KEY);if(['auto','on','off'].includes(cached))state.mode=cached;console.warn('Battle theme:',e)}apply()}
  function subscribe(){try{sb.channel('battle-theme-global-v23').on('postgres_changes',{event:'UPDATE',schema:'public',table:'battle_theme_settings',filter:'id=eq.true'},payload=>{const mode=payload.new&&payload.new.mode;if(['auto','on','off'].includes(mode)){state.mode=mode;localStorage.setItem(CACHE_KEY,mode);apply()}}).subscribe()}catch(e){console.warn('Battle theme realtime:',e)}}
  async function save(mode){try{const {data:u}=await sb.auth.getUser();const {error}=await sb.from('battle_theme_settings').update({mode,updated_at:new Date().toISOString(),updated_by:u.user.id}).eq('id',true);if(error)throw error;state.mode=mode;localStorage.setItem(CACHE_KEY,mode);apply();alert('Tema Battle: '+(mode==='auto'?'automático':mode==='on'?'ativado agora':'desativado'))}catch(e){alert(e.message||e)}}
  function ui(){if(!document.querySelector('#battleStyle')){const s=document.createElement('style');s.id='battleStyle';s.textContent=css;document.head.appendChild(s)}if(!document.querySelector('#battleBanner')){const n=document.createElement('div');n.id='battleBanner';n.innerHTML='<b>⚔ TEMPOS DE BATTLE ATIVO</b>';const nav=document.querySelector('#nav');nav&&nav.parentNode.insertBefore(n,nav)}apply()}
  const boot=setInterval(()=>{if(typeof sb!=='undefined'&&typeof S!=='undefined'&&S.profile){clearInterval(boot);ui();load();subscribe();setInterval(()=>{apply();load()},30000)}},250);
})();
